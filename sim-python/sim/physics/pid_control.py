...
class PID_Controller(object):
    ...
    def get_correction(self, target, actual, dt=1):
        '''
...
class Demand_PID_Controller(PID_Controller):
    ...
    def get_correction(self, sensorValue, demandValue, timestep=1):
        '''
        ...
        if abs(demandValue) < self.noise_threshold:
            ...
            correction = PID_Controller.get_correction(self, self.target, sensorValue, timestep)
...
class GPS_PID_Controller(PID_Controller):
    ...
    def get_correction(self, targetValue, sensorValue, dt=1):
        '''
...
class Stability_PID_Controller(PID_Controller):
    ...
    def get_correction(self, actualAngle, timestep=1):
        '''
        Returns current PID correction based on IMU angle in radians.
        '''

        return PID_Controller.get_correction(self, 0, actualAngle, timestep)
...
class Yaw_PID_Controller(Demand_PID_Controller):
    ...
    def get_correction(self, yawAngle, yawDemand, timestep=1):
        '''
        Returns current PID correction based on yaw angle in radians value and demand value in interval [-1,+1].
        '''
        
        correction =  Demand_PID_Controller.get_correction(self, -yawAngle, yawDemand, timestep)
...
class Hover_PID_Controller(PID_Controller):
    ...
    def get_correction(self, position, target=None, timestep=1):
        ...
        correction = PID_Controller.get_correction(self, target, position, timestep) \
                     if target \
                     else PID_Controller.get_correction(self, 0, velocity, timestep) \
